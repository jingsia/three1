/*************************************************************************
	...
	> Author: yangjx
	> Mail: yangjx@126.com 
	> Created Time: Tue 08 Nov 2016 06:03:47 PM CST
 ************************************************************************/
...
void readcb(struct bufferevent *bev, void *ctx)
{
	...
	while(1)
	{
		...
		else
		{
			...
			if(strstr(request_line, "over") != NULL) //用于判断是不是一条消息结束
			{
...
void do_accept(evutil_socket_t listener, short event, void *arg)
{
	...
	else
	{
		...
		//启用读写事件,其实是调用了event_add将相应读写事件加入事件监听队列poll。正如文档所说，如果相应事件不置为true，buffervent是不会读写数据的
