...
function Task_00000501(npcId)
	...
	elseif task:GetTaskSubmitNpc(501) == npcId then
		if Task_Submit_00000501() then
			...
			action.m_ActionMsg = task_msg_592;
		elseif task:HasAcceptedTask(501) then
			action.m_ActionType = 0x0001;
			action.m_ActionID = 501
			action.m_ActionToken = 0;
			action.m_ActionStep = 0;
			action.m_ActionMsg = task_msg_593;
		end
	end
	return action;
end
...
function Task_00000501_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = GetPlayerName(GetPlayer())..task_msg_594;
	action.m_ActionMsg = task_msg_595;
	return action;
end
...
function Task_00000501_submit(itemId, itemNum)
	...
	if not player:GetTaskMgr():SubmitTask(501) then
		return false;
	end
